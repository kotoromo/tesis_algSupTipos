...
\subsection{Teoría de tipos dependientes}
    ...
    Supongamos que tenemos dos flechas:
    $$
        f : A \rightarrow B\quad g : B \rightarrow C
    $$

    y queremos definir la composición de estas:
    $$
        \mathrm{comp}(f,g) : A \rightarrow C
    $$
    Así, un punto de partida es pensar en el tipo que debería tener nuestro
    objeto deseado.
    \begin{center}
        \AxiomC{$?$}
    ...
    excepción. Más aún, la composición de funciones es tal que dado un elemento
    del dominio de $f$ este se corresponde con un elemento del contradominio de
    $g$ de modo que $g(f(x))$ resulta una elección natural para la regla de
    correspondencia.
    \begin{center}
        \AxiomC{$?$}
    ...
    este tipo, donde el elemento de entrada debe ser de tipo $A$ y el elemento
    de salida debe ser de tipo $C$.
    \begin{center}
        \AxiomC{$\Gamma, ? \vdash x : A$}
        \AxiomC{$\Gamma, ? \vdash g(f(x)) : C$}
        \BinaryInfC{$\Gamma, x : A \vdash g(f(x)) : C$}
        \UnaryInfC{$\Gamma \vdash \lambda\ x\ .\ g(f(x)) : A \rightarrow C$}
        \DisplayProof
    \end{center}
    Ahora bien, debemos probar que en efecto $\Gamma, x : A \vdash g(f(x)) : C$.
    Por la regla de elemento genérico facilmente podemos deducir desde 
